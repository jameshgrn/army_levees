...
class MultiProfilePlotter:
    ...
    def plot_degradation_profiles(self, data: pd.DataFrame, show: bool = False) -> None:
    ...
    def plot_stable_profiles(self, data: pd.DataFrame, show: bool = False) -> None:
    ...
    def plot_aggradation_profiles(self, data: pd.DataFrame, show: bool = False) -> None:
    ...
    def plot_significant_profiles(self, data: pd.DataFrame, show: bool = False) -> None:
    ...
    def plot_non_significant_profiles(self, data: pd.DataFrame, show: bool = False) -> None:
    ...
    def _create_profile_plot(self, data, title, fig_num, systems_subset):
        ...
        """
        fig, axes = plt.subplots(7, 8, figsize=(15, 9))
    ...
    try:
        ...
        if data:
            ...
            if args.type == 'all':
                ...
                significant_data = all_data[all_data['system_id'].isin(classifications['significant'])]
                if not significant_data.empty:
                    logger.info("Plotting significant changes...")
                    plotter.plot_significant_profiles(significant_data, show=args.show)

                # Plot non-significant changes
                non_significant_data = all_data[all_data['system_id'].isin(classifications['non_significant'])]
                if not non_significant_data.empty:
                    logger.info("Plotting non-significant changes...")
                    plotter.plot_non_significant_profiles(non_significant_data, show=args.show)

            elif args.type == 'significant':
                ...
                # Plot only non-significant changes
                classifications = classify_levees(all_data, args.summary_dir)
                non_significant_data = all_data[all_data['system_id'].isin(classifications['non_significant'])]
                if not non_significant_data.empty:
                    logger.info("Plotting non-significant changes...")
