...
def plot_system_track_map(plot_df, system_id):
    ...
    def create_subplots(basemap_provider):
        ...
        # Plot the elevation profiles on the second subplot with both lines and dots
        nld_data = system_data[system_data['source'] == 'nld']
        tep_data = system_data[system_data['source'] == 'tep']
        
        # Plot NLD data with blue line and dots
        ax2.plot(nld_data['distance_along_track'], nld_data['elevation'], 'b-', label='NLD', linewidth=1)
        ax2.plot(nld_data['distance_along_track'], nld_data['elevation'], 'bo', markersize=3)
        
        # Plot TEP data with red line and dots
        ax2.plot(tep_data['distance_along_track'], tep_data['elevation'], 'r-', label='TEP', linewidth=1)
        ax2.plot(tep_data['distance_along_track'], tep_data['elevation'], 'ro', markersize=3)
        ...
        ax2.legend()
        ax2.grid(True, linestyle='--', alpha=0.7)  # Added grid for better readability
...
def plot_random_cases(filtered_plot_df, num_cases=10):
    # Get unique system IDs
    unique_system_ids = filtered_plot_df['system_id'].unique()
    
    # Randomly select num_cases system IDs
    selected_system_ids = random.sample(list(unique_system_ids), num_cases)
    
    for system_id in selected_system_ids:
        print(f"Plotting system ID: {system_id}")
        plot_system_track_map(filtered_plot_df, system_id)
        plt.show(block=True)  # This will block until the plot window is closed
        plt.close()  # Ensure the plot is fully closed after examination
...
if __name__ == "__main__":
    ...
    '''plot only one case'''
    plot_system_track_map(filtered_plot_df, 5705000030) # 4205000009 (might), 6005000545 is a real erosion case

    '''plot 10 random cases'''
    # # Plot 10 random cases
    # plot_random_cases(filtered_plot_df, num_cases=30)

    # print("All random cases have been plotted and examined.")
