...
def perform_dbscan_clustering(data_folder, eps=0.5, min_samples=5):
    stats_file = f'{data_folder}/elevation_difference_stats.csv'
    
    if not os.path.exists(stats_file):
        # If the stats file doesn't exist, we need to create it
        print("Statistics file not found. Generating statistics...")
        df_stats = generate_statistics(data_folder)
    ...
    features = ['mean', 'median', 'std', 'rmse', 'mse', 'kurtosis']
    X = df_stats[features]
    ...
    df_stats['cluster'] = dbscan.fit_predict(X_scaled)
    
    return df_stats

def generate_statistics(data_folder):
    # Get all NLD files
    nld_files = [f for f in os.listdir(data_folder) if '_nld_' in f and f.endswith('.parquet')]
...
def plot_dbscan_clusters(df_stats, output_file):
    fig, axs = plt.subplots(6, 5, figsize=(28, 30))  # Increased width to accommodate colorbar
    fig.suptitle('DBSCAN Clustering of Elevation Difference Statistics', fontsize=16)
    ...
    for row, row_param in enumerate(row_params):
        ...
        for col, col_param in enumerate(other_params):
            ax = axs[row, col]
            scatter = ax.scatter(df_stats[row_param], df_stats[col_param], 
                                 c=df_stats['cluster'], cmap=cmap, alpha=0.7)
            ...
            for cluster in clusters:
                if cluster != -1:  # -1 is the noise cluster in DBSCAN
                    cluster_points = df_stats[df_stats['cluster'] == cluster]
                    ...
                    if len(cluster_points) >= 2:
                        try:
                            ...
                            # Create and add the ellipse patch
                            center = cluster_points[[row_param, col_param]].mean()
...
# Perform DBSCAN clustering
df_stats_dbscan = perform_dbscan_clustering('data')

# Plot DBSCAN clusters
plot_dbscan_clusters(df_stats_dbscan, 'plots/elevation_difference_dbscan.png')
...
print("DBSCAN clustering plot has been generated and saved as 'elevation_difference_dbscan.png' in the 'plots' folder.")