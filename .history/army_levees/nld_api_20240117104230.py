...
for system_id in usace_system_ids[:4]:
    ...
    data = get_request(geojson_download_url)
    gdf = gpd.GeoDataFrame(data, geometry=gpd.GeoSeries(data['geometry']), crs='EPSG:4269')
    seg_id = gdf['segmentId'].iloc[0]
    segment_info_url = f'https://levees.sec.usace.army.mil
    elevation_data = []
