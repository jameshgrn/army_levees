...
# Assuming 'profile_data' is the JSON object you've received from the API
arcs = profile_data['geometry']['arcs']

# Extract the coordinates from the first arc (assuming there's only one arc for simplicity)
coordinates = arcs[0]

# Create a LineString from the coordinates, ignoring the fourth element in each coordinate tuple
line = LineString([(x, y) for x, y, z, m in coordinates])

# Create a GeoDataFrame with the LineString
profile_gdf = gpd.GeoDataFrame([{'geometry': line}], crs='EPSG:4269')
