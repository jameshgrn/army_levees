...
for system_id in usace_system_ids[:15]:
    ...
    try:
        ...
        # Convert the properties in the geojson feature to columns in the GeoDataFrame
        for key, value in geojson_feature['properties'].items():
            profile_gdf[key] = value
    
        profile_gdf = gpd.GeoDataFrame([{'geometry': geometry}], crs='EPSG:3857')
        profile_gdf = profile_gdf.to_crs(epsg=4269)
        print(profile_gdf.columns)
